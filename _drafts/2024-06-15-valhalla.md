...
struct ObjAndLong$Block4 {
    jobject obj0, obj1, obj2, obj3;
    jlong l0, l1, l2, l3;
}
```

这样原本每个 `ObjAndLong` 实例在数组中的大小是 16 字节，四个就要占用 64 字节，但像上面这样重排后就只需要 48 字节。
...
在这个例子中，`NullChannelExample` 中的字段 `field` 完全可以被展平，同时它也可以被赋值为 `null`。JVM 是通过**空通道（null channel）**实现它的。
...
可以看到，JVM 在展平 `field` 时，自动在其中插入了一个 `null_channel` 字段，它就是空通道。

`field.null_channel` 为 `0` 时，`field` 的其他字段都会被忽略，`field` 的值在 Java 中等于 `null`。JVM 创建对象时会将对象体清零，正好也会将 `null_channel` 填充为 `0`，所以 `field` 的默认值也是 `null`，和常规的引用类型字段完全一致。

JVM 并不是随意插入的空通道。你可能注意到了，上面 JVM 插入空通道的位置刚好是对齐填充的位置，所以增加这个字段没有任何额外的空间开销。这不是巧合，而是 JVM 刻意为之，JVM 会利用这些**裕量（slack）**，在尽可能不增加空间开销的情况下实现空通道。
...
如果必要的话，JVM 还有一些潜在的手段能够制造裕量。比如对于一个 `long` 类型的字段，JVM 可以随机生成一个概率上几乎不可能被用到的数 `x`，并通过位运算（比如用 `x ^ 1` 翻转最低位）生成另一个相应的数 `x'`，然后让 `x` 和 `x'` 具有同样的二进制表示，并通过一张全局的表存储它究竟是 `x` 还是 `x'`，这样就能挤出一个裕量用于实现空通道。

### 内部空通道和外部空通道

如果一个值对象的内部包含裕量，那么我们可以简单地在展平它的时候，利用它内部的裕量实现空通道。比如本节中第一个例子里的 `NameAndValue` 类：

`value record NameAndValue(String name, long value) {}`

它的 `name` 与 `value` 字段之间存在对齐填充，所以 JVM 直接利用这段填充的空间实现了空通道，这种空通道就是内部空通道。
...
它只有一个 `value` 字段，没有对齐填充，而 `long` 所有可能的值都是 `value` 的合法值，所以我们在它内部找不到任何裕量。对于这种情况，JVM 会在它周围“窃取”裕量实现空通道，这就是**外部空通道（external null channel）**。
...
如果周围也没有可以裕量可以利用，那么 JVM 会直接插入新的字段来实现空通道，此时无法做到零空间开销。
...
这种做法缩短了空通道与元素的距离，但它让数组元素的大小加倍，每个元素浪费了 7 字节，同样会影响性能。
...
但无论如何，只要值类型的变量支持 `null`，多少需要付出一定的成本，所以 Valhalla 支持标注类型为不可空（比如 `Long!`），
